...
@login_required
def Auctionlisting(request, listing_id):
    info = displaylisting(request, listing_id)
...
def watchlist(request, user_id):
    listing_ids = WatchList.objects.filter(user = request.user, watching=True).values('name')
    listing = AuctionListing.objects.filter(id__in = listing_ids)
